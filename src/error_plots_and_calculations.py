...
small_image_serial_by_factor = np.loadtxt('./time_measurements/serial_dithering_time_by_factor_small.dat')
large_image_serial_by_factor = np.loadtxt('./time_measurements/serial_dithering_time_by_factor_large.dat')

x_serial = small_image_serial_by_factor[:,0]

y_serial_small = np.mean(small_image_serial_by_factor[:,1:], axis=1)
y_serial_large = np.mean(large_image_serial_by_factor[:,1:], axis=1)
...
axis_1[0].plot(x_serial, y_serial_large, color='#2aa198', linestyle='-', label=r'$\mathbf{Ideal\;Speedup}$')
axis_1[0].set_xlabel('$\mathbf{Dithering\;Factor}$', fontdict={'color':'#000000'})
axis_1[0].set_ylabel(r'$\mathbf{Speedup\;\frac{T_{1}}{T_{p}}}$', fontdict={'color':'#000000'})
axis_1[0].set_title(r'$\mathbf{Dithering\;Factor\;vs\;}$', fontdict={'color':'#000000'})
axis_1[0].legend(loc=2)

axis_1[1].grid(visible=True)
axis_1[1].set_ylim(0,1.4)
axis_1[1].plot(number_of_threads, weak_scaling_speedup, color='#b58900', linestyle='-', label=r'$\mathbf{Measured\;Speedup}$')
axis_1[1].plot(number_of_threads, np.full_like(weak_scaling_speedup,1), color='#2aa198', linestyle='-', label=r'$\mathbf{Ideal\;Speedup}$')
axis_1[1].set_xlabel('$\mathbf{Number\;of\;Processors\;(p)}$', fontdict={'color':'#000000'})
axis_1[1].set_ylabel(r'$\mathbf{Speedup\;\frac{T_{1}}{T_{p}}}$', fontdict={'color':'#000000'})
axis_1[1].set_title(r'$\mathbf{Weak\;Scaling\;Test\;Results}$')
axis_1[1].legend(loc=3)
plt.savefig("./outputs/time_versus_factor.png", dpi=300)
