...
./generate_serial_readings_by_factor.sh "malenia_7680x4320.jpg" > "../time_measurements/serial_dithering_time_by_factor_large.dat"
...
printf "%s%sPlease Wait:%s Generating time readings for parallelized dithering algorithm on 1920x1080p image ...\n" "${BOLD}" "${YELLOW}" "${NORMAL}"
# ./generate_parallel_readings.sh "elden_ring_logo_1080.jpg" > "../time_measurements/parallel_dithering_time.dat"
printf "%s%sExecution Finished:%s Please check the output folder for the results\n\n" "${BOLD}" "${GREEN}" "${NORMAL}"

# Generating quantized and dithered images
printf "%s%sPlease Wait:%s Generating processed images for all images in ../input ...\n" "${BOLD}" "${YELLOW}" "${NORMAL}"
for filename in ../input/*.*; do
  echo "Processing $(basename "${filename}")"
  ./steinberg "$(basename "${filename}")" 0 1 0 1 > /dev/null
  ./steinberg "$(basename "${filename}")" 1 1 0 1 > /dev/null
done
printf "%s%sExecution finished:%s Please check the output folder for the results\n\n" "${BOLD}" "${GREEN}" "${NORMAL}"
