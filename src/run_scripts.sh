...
# This version is to be run in case the Qt package causes issues
...
# Make sure you commet out ALL the previous code blocks before running these
...
# factor=1
# ./generate_batch_outputs.sh "$factor"
...
# parallel=1
# threads=12
# ./generate_image.sh "$input" "$dithered" "$factor" "$parallel" "$threads"
